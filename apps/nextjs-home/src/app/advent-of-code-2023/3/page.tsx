...
const input = `........440...............418..643.....438......740.261......................................727...........................870..............
...
const digitsRe = /\d+/g
const otherThanDigitOrDot = /[^\d.]/
const part1 = input
  .split('\n')
  .map((line, i, list) => {
    const prevLine = list[i - 1] ?? fillerLine
    const nextLine = list[i + 1] ?? fillerLine
    const digits = line.matchAll(digitsRe)
    const validNumbers = Array.from(digits).map((match) => {
      const item = match[0]
      const index = match.index as number
      const startIndex = Math.max(index - 1, 0)
      const endIndex = Math.min(index + item.length + 1, line.length)
      const chunkAbove = prevLine.slice(startIndex, endIndex)
      const chunkOnLine = line.slice(startIndex, endIndex)
      const chunkBelow = nextLine.slice(startIndex, endIndex)
      // console.log(chunkAbove)
      // console.log(chunkOnLine)
      // console.log(chunkBelow)
      // console.log('--')
      const areaToConsider = chunkAbove + chunkOnLine + chunkBelow
      return areaToConsider.match(otherThanDigitOrDot) ? Number(item) : 0
    })
    ...
    // console.log('=========')
    return validNumbers.reduce(sumIt, 0)
  })
  .reduce(sumIt, 0)
console.log(part1)
...
const part2 = input
  ...
  .map((line, i, list) => {
    ...
    const validNumbers = Array.from(stars).map((match) => {
      const item = match[0]
      const index = match.index as number
      const startIndex = Math.max(index - 1, 0)
      const endIndex = Math.min(index + item.length + 1, line.length)
      const lines = [prevLine, line, nextLine]
      const chunks = lines.map((line) => line.slice(startIndex, endIndex))
      const twoDigitsInArea = /\d[^\d]\d/
      const oneDigitInArea = /\d/
      const digitsInArea = (chunk: string) =>
        chunk.match(twoDigitsInArea) ? 2 : chunk.match(oneDigitInArea) ? 1 : 0
      const whereDigitAre = chunks.map((chunk) => digitsInArea(chunk))
      const adjacentDigits = whereDigitAre.reduce(sumIt, 0)
      console.log('-')
      if (adjacentDigits === 2) {
        if (whereDigitAre.includes(2)) {
          const lineWithDigits =
            whereDigitAre[0] === 2
              ? prevLine
              : whereDigitAre[1] === 2
                ? line
                : nextLine
          console.log('two on a line')
          let lowerBound = null
          let offset = index - 1
          while (offset >= 0 && lineWithDigits[offset].match(digitsRe)) {
            lowerBound = offset
            offset--
          }
          let restOfLine = lineWithDigits.slice(lowerBound as number)
          const first = Number(
            (restOfLine.match(digitsRe) as RegExpMatchArray)[0]
          )
          restOfLine = lineWithDigits.slice(index + 1)
          const second = Number(
            (restOfLine.match(digitsRe) as RegExpMatchArray)[0]
          )
          return first * second
          ...
          let linesWithDigits = []
          for (let j = 0; j < 3; j++) {
            if (whereDigitAre[j] === 1) {
              linesWithDigits.push(lines[j])
            }
          }
          console.log('one a line')
          const numbers = linesWithDigits.map((tmpLine) => {
            let offset = index - 1
            let lowerBound = null
            while (offset >= 0 && tmpLine[offset].match(digitsRe)) {
              lowerBound = offset
              offset--
            }
            if (lowerBound === null) {
              while (!tmpLine[offset].match(digitsRe)) {
                offset++
                lowerBound = offset
              }
            }
            const restOfLine = tmpLine.slice(lowerBound as number)
            return Number((restOfLine.match(digitsRe) as RegExpMatchArray)[0])
          })
...
const Day3 = () => {
  return (
    <main>
      <h2>Day 3</h2>
      <p>Part 1: calculated is {part1}, and answer is 532331.</p>
      <p>Part 2: calculated is {part2}, and answer is 82301120.</p>
      <p>
