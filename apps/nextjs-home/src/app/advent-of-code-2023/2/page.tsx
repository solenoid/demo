...
const input = `Game 1: 7 blue, 6 green, 3 red; 3 red, 5 green, 1 blue; 1 red, 5 green, 8 blue; 3 red, 1 green, 5 blue
...
type colorType = keyof typeof maxes
const part1 = input
  .split('\n')
  .map((line) => {
    const [game, playLine] = line.split(': ')
    const plays = playLine.split('; ').map(
      (game) =>
        game.split(', ').map((color) => {
          const [count, name] = color.split(' ')
          return [Number(count), name]
        }) as Array<[number, colorType]>
    )
    const gameNumber = Number(game.replace('Game ', ''))
    const isGameValid = plays.every((play) =>
      play.every(([count, color]) => count <= maxes[color])
    )
...
const part2 = input
  ...
  .map((line) => {
    ...
    plays.forEach((play) => {
      play.forEach(([count, color]) => {
        if (count > mins[color]) {
          mins[color] = count
        }
      })
    })
    return Object.values(mins).reduce((prev, cur) => prev * cur, 1)
  })
  .reduce(sumIt, 0)
// console.log(part2)
...
const Day2 = () => {
  return (
    <main>
      <h2>Day 2</h2>
      <p>Part 1: calculated is {part1}, and answer is 2369.</p>
      <p>Part 2: calculated is {part2}, and answer is 66363.</p>
      <p>
